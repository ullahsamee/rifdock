...
create_dive_pop_hsearch_task( 
    ...
    for ( int i = 0; i <= rdd.opt.dive_resl-1; i++ ) {
        task_list.push_back(make_shared<HSearchScoreAtReslTask>(
            i,
            rdd.opt.tether_to_input_position_cut
            ));
        ...
        if (rdd.opt.hack_pack_during_hsearch) {
            ...
            task_list.push_back(make_shared<HackPackTask>( 
                i, 
                rdd.opt.global_score_cut
                ));
        }

        task_list.push_back(make_shared<HSearchFilterSortTask>(
            i,
            rdd.opt.beam_size / rdd.opt.DIMPOW2,
            rdd.opt.global_score_cut,
            i < rdd.opt.dive_resl-1
            ));

        if (rdd.opt.dump_x_frames_per_resl > 0) {
            task_list.push_back(make_shared<DumpHSearchFramesTask>(
                i,
                rdd.opt.dump_x_frames_per_resl,
                rdd.opt.dump_only_best_frames,
                rdd.opt.dump_only_best_stride,
    ...
    if ( rdd.opt.match_this_pdb != "") {
        task_list.push_back(make_shared<FilterByRmsdToThisPdbTask>(
            rdd.opt.match_this_pdb,
            rdd.opt.match_this_rmsd
            ));
    }
