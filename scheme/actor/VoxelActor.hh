...
namespace actor {
	...
	struct VoxelActor {
		...
		// bool operator<(THIS const & o) const { return std::make_pair(position_,voxels_) < std::make_pair(o.position_,o.voxels_); }
...
std::ostream & operator<<( std::ostream & out, Score_Voxel_vs_Atom<A,B> const& si ){ return out << si.name(); }
