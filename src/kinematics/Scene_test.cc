...
namespace TEST_BOUNDS {
	...
	TEST(Scene,interactions_emptyish_onebody){
		Scene s;
		...
		tie(adibeg,adiend) = s.get_interactions<ADI>();
		tie(adcbeg,adcend) = s.get_interactions<ADC>();
		// ADI adi = s.get_interaction_from_placeholder<ADI>(*adibeg);
		ASSERT_EQ( s.get_interaction_from_placeholder<ADI>(*adibeg++), ADI(0,0) );
		ASSERT_EQ( s.get_interaction_from_placeholder<ADI>(*adibeg++), ADI(0,1) );
		ASSERT_EQ( s.get_interaction_from_placeholder<ADI>(*adibeg++), ADI(1,0) );
		ASSERT_EQ( adibeg, adiend );
		ASSERT_EQ( s.get_interaction_from_placeholder<ADC>(*adcbeg++), ADC(1,'0') );
		ASSERT_EQ( adcbeg, adcend );
	...
	TEST(Scene,interactions_emptyish_onebody_sym_interaction){ // one-body should be all asym, so no effect
		Scene s;
	...
	TEST(Scene,interactions_emptyish_onebody_asym_getactor){ 
		Scene s;
		s.add_symframe(10.0);
		s.add_symframe(20.0);		
		Scene::iter_type<ADI>::type adibeg,adiend;
	...
	TEST(Scene,interactions_emptyish_onebody_sym_getactor_placeholder){ 
		Scene s;
		s.add_symframe(10.0);
		s.add_symframe(20.0);		
	...
	TEST(Scene,interactions_emptyish_onebody_sym_getactor){ 
		Scene s;
		s.add_symframe(10.0);
		s.add_symframe(20.0);		
	...
	TEST(Scene,interactions_emptyish_twobody){
		using std::make_pair;
	...
	TEST(Scene,interactions_emptyish_twobody_sym){
		using std::make_pair;
		...
		s.mutable_conformation_asym(0).add_actor(ADC(0,'0')); // still empty, because intra-body
			...
			tie(iiibeg,iiiend) = s.get_interactions<III>();
			tie(iccbeg,iccend) = s.get_interactions<ICC>();
			ASSERT_NE(iicbeg,iicend); ASSERT_EQ( s.get_interaction_from_iter(iicbeg++), make_pair(ADI( 0, 0 ),ADC(10,'0')) );
			ASSERT_NE(iicbeg,iicend); ASSERT_EQ( s.get_interaction_from_iter(iicbeg++), make_pair(ADI(10, 0 ),ADC( 0,'0')) );
			ASSERT_NE(icibeg,iciend); ASSERT_EQ( s.get_interaction_from_iter(icibeg++), make_pair(ADC( 0,'0'),ADI(10, 0 )) );
			ASSERT_NE(icibeg,iciend); ASSERT_EQ( s.get_interaction_from_iter(icibeg++), make_pair(ADC(10,'0'),ADI( 0, 0 )) );
			ASSERT_NE(iiibeg,iiiend); ASSERT_EQ( s.get_interaction_from_iter(iiibeg++), make_pair(ADI( 0, 0 ),ADI(10, 0 )) );
			ASSERT_NE(iccbeg,iccend); ASSERT_EQ( s.get_interaction_from_iter(iccbeg++), make_pair(ADC( 0,'0'),ADC(10,'0')) );
			ASSERT_EQ( iicbeg, iicend );
			ASSERT_EQ( icibeg, iciend );
			ASSERT_EQ( iiibeg, iiiend );
			ASSERT_EQ( iccbeg, iccend );
		...
		s.mutable_conformation_asym(1).add_actor(ADC(1,'0'));
			...
			ASSERT_NE(iicbeg,iicend); ASSERT_EQ( s.get_interaction_from_iter(iicbeg++), make_pair(ADI( 0, 0 ),ADC(01,'0')) );
			ASSERT_NE(iicbeg,iicend); ASSERT_EQ( s.get_interaction_from_iter(iicbeg++), make_pair(ADI( 0, 0 ),ADC(10,'0')) );
			ASSERT_NE(iicbeg,iicend); ASSERT_EQ( s.get_interaction_from_iter(iicbeg++), make_pair(ADI( 0, 0 ),ADC(11,'0')) );
			ASSERT_NE(iicbeg,iicend); ASSERT_EQ( s.get_interaction_from_iter(iicbeg++), make_pair(ADI(10, 0 ),ADC(00,'0')) );
			ASSERT_NE(iicbeg,iicend); ASSERT_EQ( s.get_interaction_from_iter(iicbeg++), make_pair(ADI(10, 0 ),ADC(01,'0')) );

			ASSERT_NE(icibeg,iciend); ASSERT_EQ( s.get_interaction_from_iter(icibeg++), make_pair(ADC( 0,'0'),ADI(10, 0 )) );
			ASSERT_NE(icibeg,iciend); ASSERT_EQ( s.get_interaction_from_iter(icibeg++), make_pair(ADC( 1,'0'),ADI( 0, 0 )) );
			ASSERT_NE(icibeg,iciend); ASSERT_EQ( s.get_interaction_from_iter(icibeg++), make_pair(ADC( 1,'0'),ADI(10, 0 )) );
			ASSERT_NE(icibeg,iciend); ASSERT_EQ( s.get_interaction_from_iter(icibeg++), make_pair(ADC(10,'0'),ADI( 0, 0 )) );
			ASSERT_NE(icibeg,iciend); ASSERT_EQ( s.get_interaction_from_iter(icibeg++), make_pair(ADC(11,'0'),ADI( 0, 0 )) );

			ASSERT_NE(iiibeg,iiiend); ASSERT_EQ( s.get_interaction_from_iter(iiibeg++), make_pair(ADI(0, 0 ),ADI(10, 0 )) );

			ASSERT_NE(iccbeg,iccend); ASSERT_EQ( s.get_interaction_from_iter(iccbeg++), make_pair(ADC(0,'0'),ADC( 1,'0')) );
			ASSERT_NE(iccbeg,iccend); ASSERT_EQ( s.get_interaction_from_iter(iccbeg++), make_pair(ADC(0,'0'),ADC(10,'0')) );
			ASSERT_NE(iccbeg,iccend); ASSERT_EQ( s.get_interaction_from_iter(iccbeg++), make_pair(ADC(0,'0'),ADC(11,'0')) );
			ASSERT_NE(iccbeg,iccend); ASSERT_EQ( s.get_interaction_from_iter(iccbeg++), make_pair(ADC(1,'0'),ADC(10,'0')) );
			ASSERT_NE(iccbeg,iccend); ASSERT_EQ( s.get_interaction_from_iter(iccbeg++), make_pair(ADC(1,'0'),ADC(11,'0')) );
		...
		s.mutable_conformation_asym(1).add_actor(ADC(1,'1'));
			...
			ASSERT_NE(iicbeg,iicend); ASSERT_EQ( s.get_interaction_from_iter(iicbeg++), make_pair(ADI( 0, 0 ),ADC(01,'0')) );
			ASSERT_NE(iicbeg,iicend); ASSERT_EQ( s.get_interaction_from_iter(iicbeg++), make_pair(ADI( 0, 0 ),ADC(01,'1')) );
			ASSERT_NE(iicbeg,iicend); ASSERT_EQ( s.get_interaction_from_iter(iicbeg++), make_pair(ADI( 0, 0 ),ADC(10,'0')) );
			ASSERT_NE(iicbeg,iicend); ASSERT_EQ( s.get_interaction_from_iter(iicbeg++), make_pair(ADI( 0, 0 ),ADC(11,'0')) );
			ASSERT_NE(iicbeg,iicend); ASSERT_EQ( s.get_interaction_from_iter(iicbeg++), make_pair(ADI( 0, 0 ),ADC(11,'1')) );
			ASSERT_NE(iicbeg,iicend); ASSERT_EQ( s.get_interaction_from_iter(iicbeg++), make_pair(ADI(10, 0 ),ADC(00,'0')) );
			ASSERT_NE(iicbeg,iicend); ASSERT_EQ( s.get_interaction_from_iter(iicbeg++), make_pair(ADI(10, 0 ),ADC(01,'0')) );
			ASSERT_NE(iicbeg,iicend); ASSERT_EQ( s.get_interaction_from_iter(iicbeg++), make_pair(ADI(10, 0 ),ADC(01,'1')) );

			ASSERT_NE(icibeg,iciend); ASSERT_EQ( s.get_interaction_from_iter(icibeg++), make_pair(ADC( 0,'0'),ADI(10, 0 )) );
			ASSERT_NE(icibeg,iciend); ASSERT_EQ( s.get_interaction_from_iter(icibeg++), make_pair(ADC( 1,'0'),ADI( 0, 0 )) );
			ASSERT_NE(icibeg,iciend); ASSERT_EQ( s.get_interaction_from_iter(icibeg++), make_pair(ADC( 1,'1'),ADI( 0, 0 )) );
			ASSERT_NE(icibeg,iciend); ASSERT_EQ( s.get_interaction_from_iter(icibeg++), make_pair(ADC( 1,'0'),ADI(10, 0 )) );
			ASSERT_NE(icibeg,iciend); ASSERT_EQ( s.get_interaction_from_iter(icibeg++), make_pair(ADC( 1,'1'),ADI(10, 0 )) );
			ASSERT_NE(icibeg,iciend); ASSERT_EQ( s.get_interaction_from_iter(icibeg++), make_pair(ADC(10,'0'),ADI( 0, 0 )) );
			ASSERT_NE(icibeg,iciend); ASSERT_EQ( s.get_interaction_from_iter(icibeg++), make_pair(ADC(11,'0'),ADI( 0, 0 )) );
			ASSERT_NE(icibeg,iciend); ASSERT_EQ( s.get_interaction_from_iter(icibeg++), make_pair(ADC(11,'1'),ADI( 0, 0 )) );

			ASSERT_NE(iiibeg,iiiend); ASSERT_EQ( s.get_interaction_from_iter(iiibeg++), make_pair(ADI(0, 0 ),ADI(10, 0 )) );

			ASSERT_NE(iccbeg,iccend); ASSERT_EQ( s.get_interaction_from_iter(iccbeg++), make_pair(ADC(0,'0'),ADC( 1,'0')) );
			ASSERT_NE(iccbeg,iccend); ASSERT_EQ( s.get_interaction_from_iter(iccbeg++), make_pair(ADC(0,'0'),ADC( 1,'1')) );
			ASSERT_NE(iccbeg,iccend); ASSERT_EQ( s.get_interaction_from_iter(iccbeg++), make_pair(ADC(0,'0'),ADC(10,'0')) );
			ASSERT_NE(iccbeg,iccend); ASSERT_EQ( s.get_interaction_from_iter(iccbeg++), make_pair(ADC(0,'0'),ADC(11,'0')) );
			ASSERT_NE(iccbeg,iccend); ASSERT_EQ( s.get_interaction_from_iter(iccbeg++), make_pair(ADC(0,'0'),ADC(11,'1')) );
			ASSERT_NE(iccbeg,iccend); ASSERT_EQ( s.get_interaction_from_iter(iccbeg++), make_pair(ADC(1,'0'),ADC(10,'0')) );
			ASSERT_NE(iccbeg,iccend); ASSERT_EQ( s.get_interaction_from_iter(iccbeg++), make_pair(ADC(1,'1'),ADC(10,'0')) );
			ASSERT_NE(iccbeg,iccend); ASSERT_EQ( s.get_interaction_from_iter(iccbeg++), make_pair(ADC(1,'0'),ADC(11,'0')) );
			ASSERT_NE(iccbeg,iccend); ASSERT_EQ( s.get_interaction_from_iter(iccbeg++), make_pair(ADC(1,'0'),ADC(11,'1')) );
			ASSERT_NE(iccbeg,iccend); ASSERT_EQ( s.get_interaction_from_iter(iccbeg++), make_pair(ADC(1,'1'),ADC(11,'0')) );
			ASSERT_NE(iccbeg,iccend); ASSERT_EQ( s.get_interaction_from_iter(iccbeg++), make_pair(ADC(1,'1'),ADC(11,'1')) );
		...
		s.mutable_conformation_asym(1).add_actor(ADI(1,0));
			...
			ASSERT_NE(iicbeg,iicend); ASSERT_EQ( s.get_interaction_from_iter(iicbeg++), make_pair(ADI( 0, 0 ),ADC( 1,'0')) );
			ASSERT_NE(iicbeg,iicend); ASSERT_EQ( s.get_interaction_from_iter(iicbeg++), make_pair(ADI( 0, 0 ),ADC( 1,'1')) );
			ASSERT_NE(iicbeg,iicend); ASSERT_EQ( s.get_interaction_from_iter(iicbeg++), make_pair(ADI( 0, 0 ),ADC(10,'0')) );
			ASSERT_NE(iicbeg,iicend); ASSERT_EQ( s.get_interaction_from_iter(iicbeg++), make_pair(ADI( 0, 0 ),ADC(11,'0')) );
			ASSERT_NE(iicbeg,iicend); ASSERT_EQ( s.get_interaction_from_iter(iicbeg++), make_pair(ADI( 0, 0 ),ADC(11,'1')) );
			ASSERT_NE(iicbeg,iicend); ASSERT_EQ( s.get_interaction_from_iter(iicbeg++), make_pair(ADI( 1, 0 ),ADC( 0,'0')) );
			ASSERT_NE(iicbeg,iicend); ASSERT_EQ( s.get_interaction_from_iter(iicbeg++), make_pair(ADI( 1, 0 ),ADC(10,'0')) );
			ASSERT_NE(iicbeg,iicend); ASSERT_EQ( s.get_interaction_from_iter(iicbeg++), make_pair(ADI( 1, 0 ),ADC(11,'0')) );
			ASSERT_NE(iicbeg,iicend); ASSERT_EQ( s.get_interaction_from_iter(iicbeg++), make_pair(ADI( 1, 0 ),ADC(11,'1')) );
			ASSERT_NE(iicbeg,iicend); ASSERT_EQ( s.get_interaction_from_iter(iicbeg++), make_pair(ADI(10, 0 ),ADC( 0,'0')) );
			ASSERT_NE(iicbeg,iicend); ASSERT_EQ( s.get_interaction_from_iter(iicbeg++), make_pair(ADI(10, 0 ),ADC( 1,'0')) );
			ASSERT_NE(iicbeg,iicend); ASSERT_EQ( s.get_interaction_from_iter(iicbeg++), make_pair(ADI(10, 0 ),ADC( 1,'1')) );
			ASSERT_NE(iicbeg,iicend); ASSERT_EQ( s.get_interaction_from_iter(iicbeg++), make_pair(ADI(11, 0 ),ADC( 0,'0')) );
			ASSERT_NE(iicbeg,iicend); ASSERT_EQ( s.get_interaction_from_iter(iicbeg++), make_pair(ADI(11, 0 ),ADC( 1,'0')) );
			ASSERT_NE(iicbeg,iicend); ASSERT_EQ( s.get_interaction_from_iter(iicbeg++), make_pair(ADI(11, 0 ),ADC( 1,'1')) );

			ASSERT_NE(icibeg,iciend); ASSERT_EQ( s.get_interaction_from_iter(icibeg++), make_pair(ADC( 0,'0'),ADI( 1, 0 )) );
			ASSERT_NE(icibeg,iciend); ASSERT_EQ( s.get_interaction_from_iter(icibeg++), make_pair(ADC( 0,'0'),ADI(10, 0 )) );
			ASSERT_NE(icibeg,iciend); ASSERT_EQ( s.get_interaction_from_iter(icibeg++), make_pair(ADC( 0,'0'),ADI(11, 0 )) );
			ASSERT_NE(icibeg,iciend); ASSERT_EQ( s.get_interaction_from_iter(icibeg++), make_pair(ADC( 1,'0'),ADI( 0, 0 )) );
			ASSERT_NE(icibeg,iciend); ASSERT_EQ( s.get_interaction_from_iter(icibeg++), make_pair(ADC( 1,'1'),ADI( 0, 0 )) );
			ASSERT_NE(icibeg,iciend); ASSERT_EQ( s.get_interaction_from_iter(icibeg++), make_pair(ADC( 1,'0'),ADI(10, 0 )) );
			ASSERT_NE(icibeg,iciend); ASSERT_EQ( s.get_interaction_from_iter(icibeg++), make_pair(ADC( 1,'1'),ADI(10, 0 )) );
			ASSERT_NE(icibeg,iciend); ASSERT_EQ( s.get_interaction_from_iter(icibeg++), make_pair(ADC( 1,'0'),ADI(11, 0 )) );
			ASSERT_NE(icibeg,iciend); ASSERT_EQ( s.get_interaction_from_iter(icibeg++), make_pair(ADC( 1,'1'),ADI(11, 0 )) );
			ASSERT_NE(icibeg,iciend); ASSERT_EQ( s.get_interaction_from_iter(icibeg++), make_pair(ADC(10,'0'),ADI( 0, 0 )) );
			ASSERT_NE(icibeg,iciend); ASSERT_EQ( s.get_interaction_from_iter(icibeg++), make_pair(ADC(10,'0'),ADI( 1, 0 )) );
			ASSERT_NE(icibeg,iciend); ASSERT_EQ( s.get_interaction_from_iter(icibeg++), make_pair(ADC(11,'0'),ADI( 0, 0 )) );
			ASSERT_NE(icibeg,iciend); ASSERT_EQ( s.get_interaction_from_iter(icibeg++), make_pair(ADC(11,'1'),ADI( 0, 0 )) );
			ASSERT_NE(icibeg,iciend); ASSERT_EQ( s.get_interaction_from_iter(icibeg++), make_pair(ADC(11,'0'),ADI( 1, 0 )) );
			ASSERT_NE(icibeg,iciend); ASSERT_EQ( s.get_interaction_from_iter(icibeg++), make_pair(ADC(11,'1'),ADI( 1, 0 )) );

			ASSERT_NE(iiibeg,iiiend); ASSERT_EQ( s.get_interaction_from_iter(iiibeg++), make_pair(ADI(0, 0 ),ADI( 1, 0 )) );
			ASSERT_NE(iiibeg,iiiend); ASSERT_EQ( s.get_interaction_from_iter(iiibeg++), make_pair(ADI(0, 0 ),ADI(10, 0 )) );
			ASSERT_NE(iiibeg,iiiend); ASSERT_EQ( s.get_interaction_from_iter(iiibeg++), make_pair(ADI(0, 0 ),ADI(11, 0 )) );
			ASSERT_NE(iiibeg,iiiend); ASSERT_EQ( s.get_interaction_from_iter(iiibeg++), make_pair(ADI(1, 0 ),ADI(10, 0 )) );
			ASSERT_NE(iiibeg,iiiend); ASSERT_EQ( s.get_interaction_from_iter(iiibeg++), make_pair(ADI(1, 0 ),ADI(11, 0 )) );
	...
	TEST(Scene,interactions_twobody_homo){
		using std::make_pair;
	...
	TEST(Scene,interactions_twobody_homo_sym){
		using std::make_pair;
...
// 	// 		boost::tie(a1,a2) = s.get_interaction_from_placeholder<Interaction>(ip);
