...
using numeric::X1dim;
	...
	TEST(SceneIterator,interactions_emptyish_onebody_sym_interaction){ // one-body should be all asym, so no effect
		Scene s;
	...
	TEST(SceneIterator,interactions_emptyish_onebody_sym_getactor){ 
		...
		std::ostringstream out;
		BOOST_FOREACH( ADI a, s.get_actors<ADI>() ) out << a << endl;
		BOOST_FOREACH( ADC a, s.get_actors<ADC>() ) out << a << endl;		
		out << s << endl;
		// cout << out.str();
	}
	...
	TEST(SceneIterator,interactions_emptyish_twobody){
		...
		s.mutable_conformation_asym(0).add_actor(ADC(0,'0')); // still empty, because intra-body
		...
		tie(iicbeg,iicend) = s.get_interactions<IIC>();
		ASSERT_NE(iicbeg,iicend); ASSERT_EQ( s.get_interaction<IIC>(*iicbeg++), make_pair(ADI(0,0),ADC(1,'0')) );
		ASSERT_NE(iicbeg,iicend); ASSERT_EQ( s.get_interaction<IIC>(*iicbeg++), make_pair(ADI(0,0),ADC(1,'1')) );
		ASSERT_NE(iicbeg,iicend); ASSERT_EQ( s.get_interaction<IIC>(*iicbeg++), make_pair(ADI(1,0),ADC(0,'0')) );
		ASSERT_NE(iicbeg,iicend); ASSERT_EQ( s.get_interaction<IIC>(*iicbeg++), make_pair(ADI(1,1),ADC(0,'0')) );
		ASSERT_EQ( iicbeg, iicend );
		tie(icibeg,iciend) = s.get_interactions<ICI>();
		ASSERT_NE(icibeg,iciend); ASSERT_EQ( s.get_interaction<ICI>(*icibeg++), make_pair(ADC(0,'0'),ADI(1,0)) );
		ASSERT_NE(icibeg,iciend); ASSERT_EQ( s.get_interaction<ICI>(*icibeg++), make_pair(ADC(0,'0'),ADI(1,1)) );
		ASSERT_NE(icibeg,iciend); ASSERT_EQ( s.get_interaction<ICI>(*icibeg++), make_pair(ADC(1,'0'),ADI(0,0)) );
		ASSERT_NE(icibeg,iciend); ASSERT_EQ( s.get_interaction<ICI>(*icibeg++), make_pair(ADC(1,'1'),ADI(0,0)) );
		ASSERT_EQ( icibeg, iciend );
		tie(iiibeg,iiiend) = s.get_interactions<III>();
		ASSERT_NE(iiibeg,iiiend); ASSERT_EQ( s.get_interaction<III>(*iiibeg++), make_pair(ADI(0,0),ADI(1,0)) );
		ASSERT_NE(iiibeg,iiiend); ASSERT_EQ( s.get_interaction<III>(*iiibeg++), make_pair(ADI(0,0),ADI(1,1)) );
		ASSERT_EQ( iiibeg, iiiend );
		tie(iccbeg,iccend) = s.get_interactions<ICC>();
		ASSERT_NE(iccbeg,iccend); ASSERT_EQ( s.get_interaction<ICC>(*iccbeg++), make_pair(ADC(0,'0'),ADC(1,'0')) );
		ASSERT_NE(iccbeg,iccend); ASSERT_EQ( s.get_interaction<ICC>(*iccbeg++), make_pair(ADC(0,'0'),ADC(1,'1')) );
		ASSERT_EQ( iccbeg, iccend );
	...
	TEST(SceneIterator,interactions_emptyish_twobody_sym){
		...
		s.mutable_conformation_asym(0).add_actor(ADC(0,'0')); // still empty, because intra-body
			...
			tie(iiibeg,iiiend) = s.get_interactions<III>();
			tie(iccbeg,iccend) = s.get_interactions<ICC>();
			ASSERT_NE(iicbeg,iicend); ASSERT_EQ( s.get_interaction_absolute(iicbeg++), make_pair(ADI( 0, 0 ),ADC(10,'0')) );
			ASSERT_NE(iicbeg,iicend); ASSERT_EQ( s.get_interaction_absolute(iicbeg++), make_pair(ADI(10, 0 ),ADC( 0,'0')) );
			ASSERT_NE(icibeg,iciend); ASSERT_EQ( s.get_interaction_absolute(icibeg++), make_pair(ADC( 0,'0'),ADI(10, 0 )) );
			ASSERT_NE(icibeg,iciend); ASSERT_EQ( s.get_interaction_absolute(icibeg++), make_pair(ADC(10,'0'),ADI( 0, 0 )) );
			ASSERT_NE(iiibeg,iiiend); ASSERT_EQ( s.get_interaction_absolute(iiibeg++), make_pair(ADI( 0, 0 ),ADI(10, 0 )) );
			ASSERT_NE(iccbeg,iccend); ASSERT_EQ( s.get_interaction_absolute(iccbeg++), make_pair(ADC( 0,'0'),ADC(10,'0')) );
			ASSERT_EQ( iicbeg, iicend );
			ASSERT_EQ( icibeg, iciend );
			ASSERT_EQ( iiibeg, iiiend );
			ASSERT_EQ( iccbeg, iccend );
		...
		s.mutable_conformation_asym(1).add_actor(ADC(1,'0'));
			...
			ASSERT_NE(iicbeg,iicend); ASSERT_EQ( s.get_interaction_absolute(iicbeg++), make_pair(ADI( 0, 0 ),ADC(01,'0')) );
			ASSERT_NE(iicbeg,iicend); ASSERT_EQ( s.get_interaction_absolute(iicbeg++), make_pair(ADI( 0, 0 ),ADC(10,'0')) );
			ASSERT_NE(iicbeg,iicend); ASSERT_EQ( s.get_interaction_absolute(iicbeg++), make_pair(ADI( 0, 0 ),ADC(11,'0')) );
			ASSERT_NE(iicbeg,iicend); ASSERT_EQ( s.get_interaction_absolute(iicbeg++), make_pair(ADI(10, 0 ),ADC(00,'0')) );
			ASSERT_NE(iicbeg,iicend); ASSERT_EQ( s.get_interaction_absolute(iicbeg++), make_pair(ADI(10, 0 ),ADC(01,'0')) );

			ASSERT_NE(icibeg,iciend); ASSERT_EQ( s.get_interaction_absolute(icibeg++), make_pair(ADC( 0,'0'),ADI(10, 0 )) );
			ASSERT_NE(icibeg,iciend); ASSERT_EQ( s.get_interaction_absolute(icibeg++), make_pair(ADC( 1,'0'),ADI( 0, 0 )) );
			ASSERT_NE(icibeg,iciend); ASSERT_EQ( s.get_interaction_absolute(icibeg++), make_pair(ADC( 1,'0'),ADI(10, 0 )) );
			ASSERT_NE(icibeg,iciend); ASSERT_EQ( s.get_interaction_absolute(icibeg++), make_pair(ADC(10,'0'),ADI( 0, 0 )) );
			ASSERT_NE(icibeg,iciend); ASSERT_EQ( s.get_interaction_absolute(icibeg++), make_pair(ADC(11,'0'),ADI( 0, 0 )) );

			ASSERT_NE(iiibeg,iiiend); ASSERT_EQ( s.get_interaction_absolute(iiibeg++), make_pair(ADI(0, 0 ),ADI(10, 0 )) );

			ASSERT_NE(iccbeg,iccend); ASSERT_EQ( s.get_interaction_absolute(iccbeg++), make_pair(ADC(0,'0'),ADC( 1,'0')) );
			ASSERT_NE(iccbeg,iccend); ASSERT_EQ( s.get_interaction_absolute(iccbeg++), make_pair(ADC(0,'0'),ADC(10,'0')) );
			ASSERT_NE(iccbeg,iccend); ASSERT_EQ( s.get_interaction_absolute(iccbeg++), make_pair(ADC(0,'0'),ADC(11,'0')) );
			ASSERT_NE(iccbeg,iccend); ASSERT_EQ( s.get_interaction_absolute(iccbeg++), make_pair(ADC(1,'0'),ADC(10,'0')) );
			ASSERT_NE(iccbeg,iccend); ASSERT_EQ( s.get_interaction_absolute(iccbeg++), make_pair(ADC(1,'0'),ADC(11,'0')) );
		...
		s.mutable_conformation_asym(1).add_actor(ADC(1,'1'));
			...
			ASSERT_NE(iicbeg,iicend); ASSERT_EQ( s.get_interaction_absolute(iicbeg++), make_pair(ADI( 0, 0 ),ADC(01,'0')) );
			ASSERT_NE(iicbeg,iicend); ASSERT_EQ( s.get_interaction_absolute(iicbeg++), make_pair(ADI( 0, 0 ),ADC(01,'1')) );
			ASSERT_NE(iicbeg,iicend); ASSERT_EQ( s.get_interaction_absolute(iicbeg++), make_pair(ADI( 0, 0 ),ADC(10,'0')) );
			ASSERT_NE(iicbeg,iicend); ASSERT_EQ( s.get_interaction_absolute(iicbeg++), make_pair(ADI( 0, 0 ),ADC(11,'0')) );
			ASSERT_NE(iicbeg,iicend); ASSERT_EQ( s.get_interaction_absolute(iicbeg++), make_pair(ADI( 0, 0 ),ADC(11,'1')) );
			ASSERT_NE(iicbeg,iicend); ASSERT_EQ( s.get_interaction_absolute(iicbeg++), make_pair(ADI(10, 0 ),ADC(00,'0')) );
			ASSERT_NE(iicbeg,iicend); ASSERT_EQ( s.get_interaction_absolute(iicbeg++), make_pair(ADI(10, 0 ),ADC(01,'0')) );
			ASSERT_NE(iicbeg,iicend); ASSERT_EQ( s.get_interaction_absolute(iicbeg++), make_pair(ADI(10, 0 ),ADC(01,'1')) );

			ASSERT_NE(icibeg,iciend); ASSERT_EQ( s.get_interaction_absolute(icibeg++), make_pair(ADC( 0,'0'),ADI(10, 0 )) );
			ASSERT_NE(icibeg,iciend); ASSERT_EQ( s.get_interaction_absolute(icibeg++), make_pair(ADC( 1,'0'),ADI( 0, 0 )) );
			ASSERT_NE(icibeg,iciend); ASSERT_EQ( s.get_interaction_absolute(icibeg++), make_pair(ADC( 1,'1'),ADI( 0, 0 )) );
			ASSERT_NE(icibeg,iciend); ASSERT_EQ( s.get_interaction_absolute(icibeg++), make_pair(ADC( 1,'0'),ADI(10, 0 )) );
			ASSERT_NE(icibeg,iciend); ASSERT_EQ( s.get_interaction_absolute(icibeg++), make_pair(ADC( 1,'1'),ADI(10, 0 )) );
			ASSERT_NE(icibeg,iciend); ASSERT_EQ( s.get_interaction_absolute(icibeg++), make_pair(ADC(10,'0'),ADI( 0, 0 )) );
			ASSERT_NE(icibeg,iciend); ASSERT_EQ( s.get_interaction_absolute(icibeg++), make_pair(ADC(11,'0'),ADI( 0, 0 )) );
			ASSERT_NE(icibeg,iciend); ASSERT_EQ( s.get_interaction_absolute(icibeg++), make_pair(ADC(11,'1'),ADI( 0, 0 )) );

			ASSERT_NE(iiibeg,iiiend); ASSERT_EQ( s.get_interaction_absolute(iiibeg++), make_pair(ADI(0, 0 ),ADI(10, 0 )) );

			ASSERT_NE(iccbeg,iccend); ASSERT_EQ( s.get_interaction_absolute(iccbeg++), make_pair(ADC(0,'0'),ADC( 1,'0')) );
			ASSERT_NE(iccbeg,iccend); ASSERT_EQ( s.get_interaction_absolute(iccbeg++), make_pair(ADC(0,'0'),ADC( 1,'1')) );
			ASSERT_NE(iccbeg,iccend); ASSERT_EQ( s.get_interaction_absolute(iccbeg++), make_pair(ADC(0,'0'),ADC(10,'0')) );
			ASSERT_NE(iccbeg,iccend); ASSERT_EQ( s.get_interaction_absolute(iccbeg++), make_pair(ADC(0,'0'),ADC(11,'0')) );
			ASSERT_NE(iccbeg,iccend); ASSERT_EQ( s.get_interaction_absolute(iccbeg++), make_pair(ADC(0,'0'),ADC(11,'1')) );
			ASSERT_NE(iccbeg,iccend); ASSERT_EQ( s.get_interaction_absolute(iccbeg++), make_pair(ADC(1,'0'),ADC(10,'0')) );
			ASSERT_NE(iccbeg,iccend); ASSERT_EQ( s.get_interaction_absolute(iccbeg++), make_pair(ADC(1,'1'),ADC(10,'0')) );
			ASSERT_NE(iccbeg,iccend); ASSERT_EQ( s.get_interaction_absolute(iccbeg++), make_pair(ADC(1,'0'),ADC(11,'0')) );
			ASSERT_NE(iccbeg,iccend); ASSERT_EQ( s.get_interaction_absolute(iccbeg++), make_pair(ADC(1,'0'),ADC(11,'1')) );
			ASSERT_NE(iccbeg,iccend); ASSERT_EQ( s.get_interaction_absolute(iccbeg++), make_pair(ADC(1,'1'),ADC(11,'0')) );
			ASSERT_NE(iccbeg,iccend); ASSERT_EQ( s.get_interaction_absolute(iccbeg++), make_pair(ADC(1,'1'),ADC(11,'1')) );
		...
		s.mutable_conformation_asym(1).add_actor(ADI(1,0));
			...
			ASSERT_NE(iicbeg,iicend); ASSERT_EQ( s.get_interaction_absolute(iicbeg++), make_pair(ADI( 0, 0 ),ADC( 1,'0')) );
			ASSERT_NE(iicbeg,iicend); ASSERT_EQ( s.get_interaction_absolute(iicbeg++), make_pair(ADI( 0, 0 ),ADC( 1,'1')) );
			ASSERT_NE(iicbeg,iicend); ASSERT_EQ( s.get_interaction_absolute(iicbeg++), make_pair(ADI( 0, 0 ),ADC(10,'0')) );
			ASSERT_NE(iicbeg,iicend); ASSERT_EQ( s.get_interaction_absolute(iicbeg++), make_pair(ADI( 0, 0 ),ADC(11,'0')) );
			ASSERT_NE(iicbeg,iicend); ASSERT_EQ( s.get_interaction_absolute(iicbeg++), make_pair(ADI( 0, 0 ),ADC(11,'1')) );
			ASSERT_NE(iicbeg,iicend); ASSERT_EQ( s.get_interaction_absolute(iicbeg++), make_pair(ADI( 1, 0 ),ADC( 0,'0')) );
			ASSERT_NE(iicbeg,iicend); ASSERT_EQ( s.get_interaction_absolute(iicbeg++), make_pair(ADI( 1, 0 ),ADC(10,'0')) );
			ASSERT_NE(iicbeg,iicend); ASSERT_EQ( s.get_interaction_absolute(iicbeg++), make_pair(ADI( 1, 0 ),ADC(11,'0')) );
			ASSERT_NE(iicbeg,iicend); ASSERT_EQ( s.get_interaction_absolute(iicbeg++), make_pair(ADI( 1, 0 ),ADC(11,'1')) );
			ASSERT_NE(iicbeg,iicend); ASSERT_EQ( s.get_interaction_absolute(iicbeg++), make_pair(ADI(10, 0 ),ADC( 0,'0')) );
			ASSERT_NE(iicbeg,iicend); ASSERT_EQ( s.get_interaction_absolute(iicbeg++), make_pair(ADI(10, 0 ),ADC( 1,'0')) );
			ASSERT_NE(iicbeg,iicend); ASSERT_EQ( s.get_interaction_absolute(iicbeg++), make_pair(ADI(10, 0 ),ADC( 1,'1')) );
			ASSERT_NE(iicbeg,iicend); ASSERT_EQ( s.get_interaction_absolute(iicbeg++), make_pair(ADI(11, 0 ),ADC( 0,'0')) );
			ASSERT_NE(iicbeg,iicend); ASSERT_EQ( s.get_interaction_absolute(iicbeg++), make_pair(ADI(11, 0 ),ADC( 1,'0')) );
			ASSERT_NE(iicbeg,iicend); ASSERT_EQ( s.get_interaction_absolute(iicbeg++), make_pair(ADI(11, 0 ),ADC( 1,'1')) );

			ASSERT_NE(icibeg,iciend); ASSERT_EQ( s.get_interaction_absolute(icibeg++), make_pair(ADC( 0,'0'),ADI( 1, 0 )) );
			ASSERT_NE(icibeg,iciend); ASSERT_EQ( s.get_interaction_absolute(icibeg++), make_pair(ADC( 0,'0'),ADI(10, 0 )) );
			ASSERT_NE(icibeg,iciend); ASSERT_EQ( s.get_interaction_absolute(icibeg++), make_pair(ADC( 0,'0'),ADI(11, 0 )) );
			ASSERT_NE(icibeg,iciend); ASSERT_EQ( s.get_interaction_absolute(icibeg++), make_pair(ADC( 1,'0'),ADI( 0, 0 )) );
			ASSERT_NE(icibeg,iciend); ASSERT_EQ( s.get_interaction_absolute(icibeg++), make_pair(ADC( 1,'1'),ADI( 0, 0 )) );
			ASSERT_NE(icibeg,iciend); ASSERT_EQ( s.get_interaction_absolute(icibeg++), make_pair(ADC( 1,'0'),ADI(10, 0 )) );
			ASSERT_NE(icibeg,iciend); ASSERT_EQ( s.get_interaction_absolute(icibeg++), make_pair(ADC( 1,'1'),ADI(10, 0 )) );
			ASSERT_NE(icibeg,iciend); ASSERT_EQ( s.get_interaction_absolute(icibeg++), make_pair(ADC( 1,'0'),ADI(11, 0 )) );
			ASSERT_NE(icibeg,iciend); ASSERT_EQ( s.get_interaction_absolute(icibeg++), make_pair(ADC( 1,'1'),ADI(11, 0 )) );
			ASSERT_NE(icibeg,iciend); ASSERT_EQ( s.get_interaction_absolute(icibeg++), make_pair(ADC(10,'0'),ADI( 0, 0 )) );
			ASSERT_NE(icibeg,iciend); ASSERT_EQ( s.get_interaction_absolute(icibeg++), make_pair(ADC(10,'0'),ADI( 1, 0 )) );
			ASSERT_NE(icibeg,iciend); ASSERT_EQ( s.get_interaction_absolute(icibeg++), make_pair(ADC(11,'0'),ADI( 0, 0 )) );
			ASSERT_NE(icibeg,iciend); ASSERT_EQ( s.get_interaction_absolute(icibeg++), make_pair(ADC(11,'1'),ADI( 0, 0 )) );
			ASSERT_NE(icibeg,iciend); ASSERT_EQ( s.get_interaction_absolute(icibeg++), make_pair(ADC(11,'0'),ADI( 1, 0 )) );
			ASSERT_NE(icibeg,iciend); ASSERT_EQ( s.get_interaction_absolute(icibeg++), make_pair(ADC(11,'1'),ADI( 1, 0 )) );

			ASSERT_NE(iiibeg,iiiend); ASSERT_EQ( s.get_interaction_absolute(iiibeg++), make_pair(ADI(0, 0 ),ADI( 1, 0 )) );
			ASSERT_NE(iiibeg,iiiend); ASSERT_EQ( s.get_interaction_absolute(iiibeg++), make_pair(ADI(0, 0 ),ADI(10, 0 )) );
			ASSERT_NE(iiibeg,iiiend); ASSERT_EQ( s.get_interaction_absolute(iiibeg++), make_pair(ADI(0, 0 ),ADI(11, 0 )) );
			ASSERT_NE(iiibeg,iiiend); ASSERT_EQ( s.get_interaction_absolute(iiibeg++), make_pair(ADI(1, 0 ),ADI(10, 0 )) );
			ASSERT_NE(iiibeg,iiiend); ASSERT_EQ( s.get_interaction_absolute(iiibeg++), make_pair(ADI(1, 0 ),ADI(11, 0 )) );
...
// 	// {
// 	// 	std::pair<Iter2,Iter2> iters2 = s.get_interactions<Interaction>();
